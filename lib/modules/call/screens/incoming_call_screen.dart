...
class _IncomingCallScreenState extends State<IncomingCallScreen> {
  final CallSignallingController controller =
      Get.find<CallSignallingController>();
        ...
        .listen((snapshot) {
          if (snapshot.exists) {
            ...
            if (status == 'missed' ||
                ...
                status == 'declined') {
              print(
                "\ud83d\udcf4 Caller cancelled or ended the call. Closing screen.",
              );
              if (mounted) {
                _stopRingtone();
                Get.back();
  ...
  Future<void> _playRingtone() async {
    try {
      await _audioPlayer.play(
        AssetSource('audio/ringtone.mp3.m4a'),
        volume: 1.0,
      );
  ...
  Future<void> _acceptCall() async {
    try {
      ...
      RTCSessionDescription? answerSDP =
          await controller.peerConnection?.getLocalDescription();
      if (answerSDP == null) {
 ...
 Future<void> _declineCall() async {
  try {
    await controller.declineCall(widget.callId); 
